...
> **激活条件**：仅当调用 skill 明确要求执行 `<zh_shadow_audit>` 时启用。不得仅因加载了 `writing_rules.md` 就自动执行本块。

这不是英译中。英文科研写作只作为内部逻辑标尺；不得生成或输出中间英文全文。

### 审计方法

1. 逐段识别实际陈述的事实、假设、方法、动作主体、对象、条件、因果关系、比较关系和结论。
2. 内部判断：如果这些内容写成规范英文科研论文，哪些主语、动作、对象、条件和逻辑关系必须明确；再据此检查中文是否准确、完整。
3. 检查并按需改写：
   - 抽象名词代替具体含义；
   - 没有实际信息的修饰语和泛化表达；
   - AI 高频模板词及重复句式；
   ...
   - 为追求正式感增加的复杂表达；
   - “通过……从而……进而……”等机械链式句法。
4. 遇到抽象词先问“它在这里具体指什么”。能从原文和既有证据直接说出实际对象、动作或指标时，改成具体表达；不能确定时保留，不得猜测。
5. “维度、层面、口径、机制、框架、路径、体系、场景、赋能、刻画、表征、综合考虑、进一步、有效”等只是**审查触发词，不是禁词**。数学、统计学和数学建模中已经稳定使用，或在当前语境中最准确的术语必须保留。
6. 判断标准只有四项：是否准确、是否必要、是否具体、是否自然。不要为了“去 AI”牺牲专业精度。

### 保护边界

- 保留所有有效信息、结论强度、负面结果、公式含义、变量定义和专业术语；不得增加新事实、新引用或新结论。
- 不得改变数字、单位、公式、变量、引用编号、图表编号、图片路径、Markdown 结构、表格数据或代码。
- 调用者划定为只读的标题、英文段落、参考文献、附录及其他区域必须原样保留。

### 输出要求

直接更新调用者指定的原稿，只改确有问题的中文散文段落。不得输出英文翻译、审计报告、修改说明或修改清单。
</zh_shadow_audit>
